...
class SegmentsMonitor:
    ...
    async def segment_change(self):
        ...
        digit_date_edges = [Edge(self.caravelEnv.dut._id(f"gpio{digit_edge}_monitor", False)) for digit_edge in self.digit_data_pins]
        await First(*digit_en_edges, *digit_date_edges) # triggered with the first edge triggered
